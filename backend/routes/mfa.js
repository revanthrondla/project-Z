...
router.post('/verify', async (req, res) => {
  try {
    ...
    const { userId, tenantSlug, email, name, role, employeeId, clientId, positionTitle, tenantName, mustChangePw } = decoded;
    ...
    if (totpValid) {
      ...
      const token = jwt.sign(
        { id: userId, email, name, role, employeeId, clientId, positionTitle, tenantSlug, tenantName, mustChangePw },
        JWT_SECRET,
        { expiresIn: '8h' }
      );
      ...
      return res.json({
        token,
        user: { id: userId, name, email, role, employeeId, clientId, positionTitle, tenantSlug, tenantName, mustChangePw },
      });
    }
...
router.post('/email-otp/verify', async (req, res) => {
  ...
  try {
    ...
    const payload = { id: userId, email, name, role, employeeId, clientId, positionTitle, tenantSlug, tenantName, mustChangePw };
