...
router.post('/', authenticate, requireAdmin, injectTenantDb, (req, res) => {
  ...
  const existingUser = req.db.prepare('SELECT id FROM users WHERE email = ?').get(email.toLowerCase().trim());
  if (existingUser) {
    return res.status(409).json({ error: 'A user with this email already exists' });
  }

  const hash = bcrypt.hashSync(password || 'candidate123', 10);

  try {
    const candidateId = req.db.transaction(() => {
      const userResult = req.db.prepare(
        'INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)'
      ).run(name, email.toLowerCase().trim(), hash, 'candidate');

      const candidateResult = req.db.prepare(`
    ...
    const newCandidate = req.db.prepare('SELECT c.*, cl.name as client_name FROM candidates c LEFT JOIN clients cl ON c.client_id = cl.id WHERE c.id = ?').get(candidateId);
...
router.put('/:id', authenticate, injectTenantDb, (req, res) => {
  ...
  values.push(id);
  req.db.prepare(`UPDATE candidates SET ${updateFields.join(', ')} WHERE id = ?`).run(...values);

  const updated = req.db.prepare('SELECT c.*, cl.name as client_name FROM candidates c LEFT JOIN clients cl ON c.client_id = cl.id WHERE c.id = ?').get(id);
  res.json(updated);
});
...
router.delete('/:id', authenticate, requireAdmin, injectTenantDb, (req, res) => {
  const id = parseInt(req.params.id);
  const candidate = req.db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
  if (!candidate) return res.status(404).json({ error: 'Candidate not found' });

  req.db.prepare('DELETE FROM candidates WHERE id = ?').run(id);
  res.json({ message: 'Candidate deleted successfully' });
});

// GET /api/candidates/:id/stats
router.get('/:id/stats', authenticate, injectTenantDb, (req, res) => {
  const id = parseInt(req.params.id);
  if (req.user.role !== 'admin' && req.user.candidateId !== id) {
    return res.status(403).json({ error: 'Access denied' });
  }
  ...
  const monthlyHours = req.db.prepare(`
    SELECT COALESCE(SUM(hours), 0) as hours
    FROM time_entries
    WHERE candidate_id = ? AND date LIKE ? AND status != 'rejected'
  `).get(id, `${currentMonth}%`);

  const yearlyHours = req.db.prepare(`
    SELECT COALESCE(SUM(hours), 0) as hours
    FROM time_entries
    WHERE candidate_id = ? AND date LIKE ? AND status != 'rejected'
  `).get(id, `${currentYear}%`);

  const pendingEntries = req.db.prepare(`
    SELECT COUNT(*) as count FROM time_entries WHERE candidate_id = ? AND status = 'pending'
  `).get(id);

  const absenceStats = req.db.prepare(`
    SELECT type, COUNT(*) as count FROM absences WHERE candidate_id = ? AND status = 'approved'
    GROUP BY type
  `).all(id);

  const invoiceStats = req.db.prepare(`
