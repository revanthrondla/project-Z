...
router.get('/languages', (req, res) => {
  const rows = req.db.prepare('SELECT * FROM ag_languages ORDER BY language_name').all();
  res.json(rows);
});

router.post('/languages', requireAdmin, (req, res) => {
  ...
  try {
    if (is_default) {
      req.db.prepare('UPDATE ag_languages SET is_default = 0').run();
    }
    ...
    res.status(201).json(req.db.prepare('SELECT * FROM ag_languages WHERE id = ?').get(r.lastInsertRowid));
  } catch (e) {
...
router.put('/languages/:id', requireAdmin, (req, res) => {
  const { language_name, language_code, is_default } = req.body;
  if (is_default) req.db.prepare('UPDATE ag_languages SET is_default = 0').run();
  req.db.prepare(`
  ...
  res.json(req.db.prepare('SELECT * FROM ag_languages WHERE id = ?').get(req.params.id));
});

router.delete('/languages/:id', requireAdmin, (req, res) => {
  req.db.prepare('DELETE FROM ag_languages WHERE id = ?').run(req.params.id);
...
router.get('/custom-fields', (req, res) => {
  const rows = req.db.prepare('SELECT * FROM ag_custom_field_definitions ORDER BY sort_order, field_name').all();
  res.json(rows);
});

router.post('/custom-fields', requireAdmin, (req, res) => {
  ...
  res.status(201).json(req.db.prepare('SELECT * FROM ag_custom_field_definitions WHERE id = ?').get(r.lastInsertRowid));
});

router.put('/custom-fields/:id', requireAdmin, (req, res) => {
  const { field_name, field_type, applies_to, options, required, sort_order } = req.body;
  req.db.prepare(`
  ...
  res.json(req.db.prepare('SELECT * FROM ag_custom_field_definitions WHERE id = ?').get(req.params.id));
});

router.delete('/custom-fields/:id', requireAdmin, (req, res) => {
  req.db.prepare('DELETE FROM ag_custom_field_definitions WHERE id = ?').run(req.params.id);
...
router.get('/employees/:id', (req, res) => {
  const row = req.db.prepare('SELECT * FROM ag_employees WHERE id = ?').get(req.params.id);
...
router.post('/employees', requireAdmin, (req, res) => {
  ...
  try {
    ...
    res.status(201).json(req.db.prepare('SELECT * FROM ag_employees WHERE id = ?').get(r.lastInsertRowid));
  } catch (e) {
...
router.put('/employees/:id', requireAdmin, (req, res) => {
  ...
  res.json(req.db.prepare('SELECT * FROM ag_employees WHERE id = ?').get(req.params.id));
});

router.delete('/employees/:id', requireAdmin, (req, res) => {
  req.db.prepare('DELETE FROM ag_employees WHERE id = ?').run(req.params.id);
...
router.get('/products/:id', (req, res) => {
  const row = req.db.prepare('SELECT * FROM ag_products WHERE id = ?').get(req.params.id);
...
router.post('/products', (req, res) => {
  ...
  res.status(201).json(req.db.prepare('SELECT * FROM ag_products WHERE id = ?').get(r.lastInsertRowid));
});
...
router.put('/products/:id', requireAdmin, (req, res) => {
  ...
  res.json(req.db.prepare('SELECT * FROM ag_products WHERE id = ?').get(req.params.id));
});

router.delete('/products/:id', requireAdmin, (req, res) => {
  req.db.prepare('DELETE FROM ag_products WHERE id = ?').run(req.params.id);
...
router.post('/scanned-products', (req, res) => {
  const {
  ...
  res.status(201).json(req.db.prepare('SELECT * FROM ag_scanned_products WHERE id = ?').get(r.lastInsertRowid));
});

router.delete('/scanned-products/:id', requireAdmin, (req, res) => {
  req.db.prepare('DELETE FROM ag_scanned_products WHERE id = ?').run(req.params.id);
  res.json({ message: 'Scanned product removed' });
});

// Bulk offline sync — accepts array of scanned products created offline
router.post('/scanned-products/sync', (req, res) => {
  ...
  // node:sqlite uses manual BEGIN/COMMIT instead of db.transaction()
  req.db.exec('BEGIN');
  const ids = [];
  try {
...
router.get('/analytics', (req, res) => {
  ...
  const p = [];
  if (from) { whereClause += " AND date(scanned_at) >= ?"; p.push(from); }
  if (to)   { whereClause += " AND date(scanned_at) <= ?"; p.push(to); }
  ...
  const byProduct = req.db.prepare(`
    SELECT product_name,
...
router.get('/analytics/snapshots', requireAdmin, (req, res) => {
  const rows = req.db.prepare('SELECT * FROM ag_analytics_snapshots ORDER BY snapshot_date DESC').all();
  res.json(rows.map(r => ({
    ...r,
